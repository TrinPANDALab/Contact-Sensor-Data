...
while (ctr != 400):
    ...
    if (flag == 0):#bringUp
        ...
        subprocess.run(["sudo","./adxl345spi","-t","1","-f","3200","-s",str(now)+"_"+"_3200_0.csv"]) #change file name based on location (1-5)
    else: #bringDown
        wiringpi.pwmWrite(18, 178)  #write PWM pulses of 175 to pin 18, rotating end onto rod
        GPIO.output(17,GPIO.LOW)
        subprocess.run(["sudo","./adxl345spi","-t","1","-f","3200","-s",str(now)+"_"+"_3200_"+str(section)+".csv"]) #change file name based on location (1-5)
